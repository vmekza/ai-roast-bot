...
export default function Onboarding() {
  ...
  // we only need the setter, so ignore the first element
  const [, setIsLooping] = useState(true);
  const timers = useRef([]);

  useEffect(() => {
    // capture the array reference once
    ...
    function cycle() {
      ...
      currentTimers.push(stopTimer, startTimer);
    }
    ...
    // include the setter in deps to satisfy exhaustive-deps
  }, [setIsLooping]);
